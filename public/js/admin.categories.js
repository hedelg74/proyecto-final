import { showDialog } from "./showdialog.message.js";
	...
	export async function loadCategories() {
				try {
					const response = await fetch("/mant-load-category-list");
					if (!response.ok) {
...
function handlePanelActions() {
	...
	function functionSelector(e) {
		if (e.target.classList.contains("delete")) {
			if (confirm("¿Está seguro de eliminar esta categoria?")) deleteItem(e);
	...
	function editCategory(params) {
		fetch("/mant-edit-category", {
			...
			.then((data) => {
				...
				categoryId.textContent = data.data[0].id;
	...
	document.querySelector("#edit-form").addEventListener("submit", (event) => {
		...
		const categoryId = Number(document.getElementById("category-id").textContent);
	...
	async function updateCategory(formObject) {
		try {
			const response = await fetch("/mant-update-category", {
				method: "PUT",
	...
	document.getElementById('add-category').addEventListener('click', showAddDialog);
	...
	document.querySelector("#add-form").addEventListener("submit", (event) => {
		...
		const formData = new FormData(event.target);
		const formObject = Object.fromEntries(formData);
		const url = "/mant-add-category";
		addcategory(url, formObject);
	});
	
	
	async function addcategory(url, formObject) {
		try {
	...
	async function deleteCategory(id) {
		try {
			const response = await fetch("/mant-delete-category", {
				method: "DELETE",
				headers: { "Content-Type": "application/json" },
